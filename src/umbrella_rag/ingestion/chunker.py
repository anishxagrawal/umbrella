...
def generate_summary_chunks(
    ...
    Args:
        ...
        spec_meta: Dict with spec_number and series_id.
        summary_words_per_child: Number of words to take from each child.
    ...
    for section in sections_with_hierarchy:
        ...
        for child in children:
            ...
            snippet = _first_n_words(child.get("text", ""), summary_words_per_child)
            summary_parts.append(f"{child_number}: {snippet}".strip())

        summary_text = f"Section {label} - {title} covers: " + " | ".join(summary_parts)
